...
const CATEGORIES = ['Nanny & Babysitter','Housekeeper & Cleaner','Private Chef & Cook','Driver & Chauffeur','Gardener & Pool Care','Elder Care & Caregiver','Tutor & Teacher'];
const CAT_EMOJI = {'Nanny & Babysitter':'\ud83d\udc76','Housekeeper & Cleaner':'\ud83c\udfe0','Private Chef & Cook':'\ud83d\udc68\u200d\ud83c\udf73','Driver & Chauffeur':'\ud83d\ude97','Gardener & Pool Care':'\ud83c\udf3f','Elder Care & Caregiver':'\ud83c\udfe5','Tutor & Teacher':'\ud83d\udcda'};
const DEMO_PHOTOS = [
...
export default function Helpers() {
  ...
  useEffect(() => { const s = localStorage.getItem('th_lang') || 'en'; setLangState(s); }, []);
  useEffect(() => { fetchHelpers(); }, []);

  const changeLang = (l) => { setLangState(l); localStorage.setItem('th_lang', l); };
  const fetchHelpers = async () => {
    setLoading(true);
    try { const res = await fetch('/api/helpers'); const data = await res.json(); setHelpers(data.helpers || []); setIsDemo(data.demo || false); } catch (err) { console.error(err); setHelpers([]); }
    setLoading(false);
  };
  const t = T[lang];
  ...
  const getCatDisplay = (cat) => { const e = Object.entries(CAT_EMOJI).find(([k]) => cat?.toLowerCase().includes(k.toLowerCase().split(' ')[0])); return e ? e[1]+' '+cat : cat; };
  const getTranslatedCat = (cat) => { const k = Object.keys(CAT_EMOJI).find(k => cat?.toLowerCase().includes(k.toLowerCase().split(' ')[0])); if (!k) return cat; const ck = 'cat_'+{'Nanny & Babysitter':'nanny','Housekeeper & Cleaner':'housekeeper','Private Chef & Cook':'chef','Driver & Chauffeur':'driver','Gardener & Pool Care':'gardener','Elder Care & Caregiver':'elder','Tutor & Teacher':'tutor'}[k]; return t[ck] || getCatDisplay(cat); };

  return (
    <>
      <Head>
        <title>{lang === 'th' ? '\u0e04\u0e49\u0e19\u0e2b\u0e32\u0e1c\u0e39\u0e49\u0e0a\u0e48\u0e27\u0e22 \u2014 ThaiHelper' : 'Find Helpers \u2014 ThaiHelper'}</title>
        <meta name="description" content="Browse trusted household helpers in Thailand." />
      ...
      <div className={lang === 'th' ? 'lang-th' : ''}>
        ...
        <section className="helpers-filters">
          <div className="helpers-filters-inner">
            ...
            {(filterCity || filterCat || filterArea) && <button className="filter-reset-btn" onClick={resetFilters}>{t.filter_reset}</button>}
          </div>
        </section>
        {isDemo && <div className="demo-banner"><span>\u2139\ufe0f {t.demo_banner}</span></div>}
        <section className="helpers-grid-section">
          <div className="helpers-grid-inner">
            <div className="helpers-results-count"><strong>{filtered.length}</strong> {t.results}</div>
            {loading ? (
              <div className="helpers-loading"><div className="loading-spinner"></div></div>
            ) : filtered.length === 0 ? (
              <div className="helpers-empty"><h3>{t.no_results}</h3><p>{t.no_results_sub}</p><button className="btn-outline" onClick={resetFilters}>{t.filter_reset}</button></div>
            ) : (
              <div className="helpers-grid">
                {filtered.map((h, i) => (
                  <div className="helper-card" key={i}>
                    <div className="helper-card-header">
                      <div className="helper-avatar"><img src={DEMO_PHOTOS[i % DEMO_PHOTOS.length]} alt={h.name} /></div>
                    ...
                    <div className="helper-card-footer">
                      {h.phone && (
                        <div className="helper-contact-row">
                          <a className="helper-contact-btn phone" href={`tel:${h.phone.replace(/[^0-9+]/g, '')}`}>\ud83d\udcde {h.phone}</a>
                          <span className={`wa-badge ${h.hasWhatsApp ? 'wa-yes' : 'wa-no'}`}>{h.hasWhatsApp ? t.card_wa_yes : t.card_wa_no}</span>
                        </div>
                      )}
                      {h.phone && h.hasWhatsApp && (
                        <a className="helper-contact-btn whatsapp" href={`https://wa.me/${h.phone.replace(/[^0-9]/g, '')}`} target="_blank" rel="noopener noreferrer">\ud83d\udcac {t.card_whatsapp}</a>
                      )}
                      {h.email && (
                        <a className="helper-contact-btn email" href={`mailto:${h.email}`}>\u2709\ufe0f {h.email}</a>
                      )}
                      {!h.phone && !h.email && <div className="helper-no-contact">Contact info not available</div>}
                    </div>
                  </div>
                ))}
              </div>
        ...
        <footer>
          ...
          <p style={{ marginTop: '6px' }}><Link href="/privacy">{t.footer_privacy}</Link> \u00b7 <Link href="/terms">{t.footer_terms}</Link></p>
        </footer>
      </div>
    </>
  );
